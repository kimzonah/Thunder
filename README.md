...
- 저희는 상태 반환을 위한 요청을 새로 만들기로 하고, Back-end에서 '친구 아니고 요청없음:0 / 친구:1 / 로그인유저가 요청 보내고 승인대기중:2 / 로그인유저가 요청 받음:3'과 같이 요청을 반환하였습니다.
- Front-end에서는 이러한 요청에 대한 응답을 기준으로 유저가 친구 관계나 번개 가입 유무를 판단하여 적절한 버튼을 보이게 구현하였습니다.
</br>

## 8. 프로젝트 후기
#### 이지혜
- 짧은 시간 내에 프로젝트를 진행하게 되면서 최대한 기능 구현의 완성도를 높이자는 목표를 가지고 진행하게 되었는데, 기존의 목표를 달성했고, 수상이라는 결과까지 따라오게 되어 만족하였습니다. 다만 빠른 시일 내 완성해야 하다보니 코드의 리팩토링이나 통일성에 신경을 쓰지 못한 부분이 조금 마음에 걸렸습니다. 
